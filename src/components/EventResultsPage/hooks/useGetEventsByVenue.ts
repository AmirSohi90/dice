...
const mapLineUp = (lineup: Array<{ details: string; time: string }>) => lineup.filter(artist => artist.details !== "Doors open");

const getStartTime = (lineup: Array<{ details: string; time: string }>) => lineup.find(artist => artist.details === "Doors open")?.time || "";

const getPreviewTrack = (spotifyTracks: Array<{
    open_url: string;
    preview_url: string;
    title: string;
...
const getTicketData = (tickets: Array<{
    id: number;
    name: string;
    price: { face_value: number; fees: number; total: number };
    sold_out: boolean;
}>) => tickets.map(({id, name, price, sold_out}) => ({id, name, price: price.total, soldOut: sold_out}))
...
export const useGetEventsByVenue = (venueName: string): GetEventsByVenueResponse => {
    const queryKey = `venueName:${venueName}`
    const options = { enabled: !!venueName }
    const { data, status } = useQuery(queryKey, () => apis.getEventsByVenue(venueName), options)
    return { data: status === 'success' ? data.data.map(event => mapEventData(event)) : null}
};
